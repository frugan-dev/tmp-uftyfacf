...
RUN apt-get update \
    ...
    && if [[ -n "${WORDPRESS_MTA}" && ! "${WORDPRESS_MTA,,}" =~ ^(no|false|0)$ ]]; then \
        if [ "$WORDPRESS_MTA" = "msmtp" ]; then \
            ...
            # Install build dependencies for nullmailer
            apt-get install -y --no-install-recommends \
                build-essential \
                autoconf \
                automake \
                ...
                ca-certificates \
            # Clone nullmailer
            && git clone https://github.com/bruceg/nullmailer.git /tmp/nullmailer; \
        fi; \
    fi \
    && rm -rf /var/lib/apt/lists/*

# Build nullmailer if needed (use WORKDIR instead of cd)
WORKDIR /tmp/nullmailer
RUN if [ "$WORDPRESS_MTA" = "nullmailer" ]; then \
        chmod +x autogen.sh \
        && ./autogen.sh \
        ...
        && rm -rf /tmp/nullmailer; \
    fi

# Reset WORKDIR to root
WORKDIR /

COPY build/docker/mta-manager.sh /usr/local/bin/mta-manager.sh
COPY build/docker/msmtprc.dist /tmp/msmtprc.dist

# Method #1
COPY build/docker/sendmail-wrapper.sh /usr/local/bin/sendmail-wrapper.sh
...
RUN if [[ -n "${WORDPRESS_MTA}" && ! "${WORDPRESS_MTA,,}" =~ ^(no|false|0)$ ]]; then \
        if [ "$WORDPRESS_MTA" = "msmtp" ]; then \
            ...
            && mkdir -p /etc/nullmailer /var/spool/nullmailer \
            ...
            && chown -R root:root /etc/nullmailer /var/spool/nullmailer; \
        fi; \
    fi \
